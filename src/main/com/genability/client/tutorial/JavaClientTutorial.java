...
public class JavaClientTutorial {
	 ...
	 * there will only be one, but sometimes there will be many to choose from. As with Accounts, making a request
	 * the LSE endpoint is a three step process -- create the request, add your parameters, and then send it to
	 * the service. In this case, we'll be using a GetLsesRequest and send it to the LseService.
	 */
	 ...
	 * the correct service class. For the Accounts, endpoint, you'll use the AccountService class. Note that
	...
	public void doSavingsAnalysis(Account theAccount, int targetEnergyConsumption) {
		...
		// are three settings: "before", "solar", and "after". The "before" scenario is what happens if no solar system
		...
		// after a solar power system is installed. It does not include the cost of the solar energy. Since we don't have
		// detailed usage (or production) data for any scenario, we're going to use a typical baseline for all three of them
		...
		// This is where we tell the calculator to scale the baseline usage to a particular target amount (entered by the user).
		// Without this setting, the calculator would assume the customer's usage to be whatever the average is for the area.
		// Since we want to use the same energy consumption for both with and without solar, we use this setting for
